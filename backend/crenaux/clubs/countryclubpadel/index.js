...
async function getAvailableSlots(date, startHour = 8, endHour = 22) {
  try {
    console.log(`[Country Club Padel] Recherche de créneaux pour ${date} entre ${startHour}h et ${endHour}h`);
    
    // Appeler la fonction findAvailableSlots
    const slots = await findAvailableSlots(date);
    ...
    console.error(`[Country Club Padel] Erreur lors de la recherche:`, error);
    return [];
  }
}
...
async function testConnection() {
  try {
    ...
    if (!username || !password) {
      console.warn('[Country Club Padel] Identifiants non configurés, test de connexion ignoré');
      // Pour le monitoring, on considère que c'est OK si les identifiants ne sont pas configurés
      // car ce n'est pas critique pour le fonctionnement de l'application
      return true;
    }
    
    // Tester la connexion avec une requête HTTP simple au lieu d'utiliser Puppeteer
    // Cela évite les problèmes liés à l'exécution de Puppeteer dans un environnement de monitoring
    const axios = require('axios');
    
    // Faire une requête simple vers la page d'accueil d'OpenResa
    const response = await axios.get('https://openresa.com/', {
    ...
    // Si on arrive ici, c'est que la connexion au site est réussie
    console.log('[Country Club Padel] Test de connexion réussi');
    return true;
  } catch (error) {
    console.error(`[Country Club Padel] Test de connexion échoué:`, error);
    return false;
  }
}
